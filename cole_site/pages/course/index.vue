          ...
          <section class="fl">
            ...
            <ol class="js-tap clearfix">
              <li :class="{'current bg-green': $route.query.buyCountSort}">
                <a v-if="!$route.query.type2 || $route.query.type2 == 1" title="销量" href="javascript:void(0);" @click="searchBuyCount(2)">销量
                  <i>↑</i>
                </a>

                <a v-if="$route.query.type2 == 2" title="销量" href="javascript:void(0);" @click="searchBuyCount(1)">销量
                  <i>↓</i>
                </a>
              </li>

              <li :class="{'current bg-green': $route.query.gmtCreateSort}">
                <a v-if="!$route.query.type1 || $route.query.type1 == 1" title="创建时间" href="javascript:void(0);" @click="searchGmtCreate(2)">创建时间
                  <i>↑</i>
                </a>

                <a v-if="$route.query.type1 == 2" title="创建时间" href="javascript:void(0);" @click="searchGmtCreate(1)">创建时间
                  <i>↓</i>
                </a>
              </li>
...
export default {
  ...
  async asyncData(page) {
    ...
    // 从url地址栏中获取查询参数,对页面响应部分的内容进行高亮显示
    const searchObj = {}
    // 从url地址栏中获取查询参数
    ...
    searchObj.type = query.type || '' // 1：价格正序，2：倒序
    searchObj.type1 = query.type1 || '' // 1：创建时间正序，2：倒序
    searchObj.type2 = query.type2 || '' // 1：销量正序，2：倒序
  ...
  methods: {
    ...
    searchPrice(type) {
      ...
      const obj = {
        subjectParentId: this.searchObj.subjectParentId,
        subjectId: this.searchObj.subjeactId,
        priceSort: 1,
        type: type
      }
      const querys = querystring.stringify(obj)
